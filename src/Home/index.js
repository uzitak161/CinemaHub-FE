...
function Home() {
    ...
    const fetchReviewData = async (
        ...
        if (removeDuplicatesByMovieId) {
            console.log("Before removing duplicates: ", reviews);
            reviews = reviews.reduce((accumulator, current) => {
                if (!accumulator.find((item) => item.movieId.omdbId === current.movieId.omdbId)) {
                    accumulator.push(current);
                }
                return accumulator;
            }, []);
            console.log("After removing duplicates: ", reviews);
        }
        ...
        if (onlyTopRated) {
            reviews = reviews.filter((review) => review.starRating >= 4);
        }
